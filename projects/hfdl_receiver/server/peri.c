...
FILE* fopen_unbuf(const char *_FileName, const char *_Mode) {
  FILE* f = fopen(_FileName, _Mode);
  setbuf(f, NULL);
  return f;
}
...
static void pinMode(uint32_t pin_num, char *mode)
{
    ...
    sysfs_export = fopen_unbuf("/sys/class/gpio/export", "w");
    ...
    sysfs_direction = fopen_unbuf(path, "w");
    ...
    handlers[pin_num] = fopen_unbuf(path, "w");
}
...
void att_cleanUp()
{
    ...
    FILE *sysfs_unexport = fopen_unbuf("/sys/class/gpio/unexport", "w");
    for (int i = 0; i < 3; i++) {
        int pin_num = pins[i];
